...
const AdminsPage = ({ t, theme, user }) => {
  ...
  useEffect(() => {
    ...
    const fetchAdmins = async () => {
      try {
        const result = await apiFetch(`/api/v1/admin/${projectId}/list`);
        if (result.ok) {
  ...
  // status → style map for this page's vocabulary (active/inactive/pending)
  const adminStatusConfig = {
    active: { color: primaryColor, dotColor: primaryColor },
    pending: { color: "#faad14", dotColor: "#faad14" },
    default: {
      color: isDark ? "#666" : "#9ca3af",
      dotColor: isDark ? "#444" : "#e5e7eb",
    },
  };

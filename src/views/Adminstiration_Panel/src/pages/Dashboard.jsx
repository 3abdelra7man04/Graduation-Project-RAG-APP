...
const DashboardPage = ({ t, theme, user }) => {
  ...
  const uploadedDocs = useApiData(`/api/v1/dashboard/uploaded_documents/${projectId}`, {
    transform: (data) => {
      if (!data || data.total_documents === undefined) return null;
  ...
  const uploadedDocsStats = uploadedDocs.data ?? {
    total: "2,769",
    delta: "+12% " + t("thisWeek").replace("{n}", ""),
  };

  const queriesAnswered = useApiData(`/api/v1/dashboard/queries_answered/${projectId}`, {
    transform: (data) => {
      if (!data || data.total_answered === undefined) return null;
      const pctText = data.percentage_change >= 0 ? `+${data.percentage_change}%` : `${data.percentage_change}%`;
      return {
  ...
  const queriesAnsweredStats = queriesAnswered.data ?? {
    total: "14,382",
    delta: "+8% " + t("thisWeek").replace("{n}", ""),
  };

  const avgLatency = useApiData(`/api/v1/dashboard/avg_latency/${projectId}`, {
    transform: (data) => {
  ...
  const avgLatencyStats = avgLatency.data ?? {
    value: "1.4s",
    delta: "-0.2s " + t("improved").replace("{n}", ""),
  };

  const failedQueries = useApiData(`/api/v1/dashboard/failed_queries/${projectId}`, {
    transform: (data) => {
      if (!data || data.total_failed === undefined) return null;
  ...
  const failedQueriesStats = failedQueries.data ?? {
    total: "47",
    delta: "+3 " + t("sincYesterday").replace("{n}", ""),
  };

  const perDay = useApiData(`/api/v1/dashboard/count_queries_this_week_per_day/${projectId}`, {
    transform: (data) => (data && data.counts) ? data.counts : null,
  });
  const queriesPerDay = perDay.data ?? {
    mon: 42, tue: 68, wed: 55, thu: 80, fri: 73, sat: 30, sun: 25,
  };

...
// Returns today's date and the date 7 days ago as "YYYY-MM-DD" strings,
// matching the format the backend sends `chat.date` in.
...
const InboxPage = ({ t, lang, theme, user }) => {
  ...
  const statsResult = useApiData(`/api/v1/monitor/stats/${projectId}`, {
    transform: (data) => {
  ...
  const monitorProjectStats = statsResult.data ?? {
    costThisMonth: "$0.0000",
    inputTokens: "0",
    outputTokens: "0",
  };

  const latencyResult = useApiData(`/api/v1/dashboard/avg_latency/${projectId}`, {
    transform: (data) => {
      if (!data || data.overall_avg_latency === undefined) return null;
      const d = data.delta_latency;
      const sign = d > 0 ? `+${d}s` : `${d}s`;
      return {
        value: `${data.overall_avg_latency}s`,
        delta: `${sign} ${isAr ? "تحسن" : "improved"}`,
      };
    },
  });
  const avgLatencyVal = latencyResult.data?.value ?? "1.4s";

  const chatsResult = useApiData(`/api/v1/chat_inbox/list/${projectId}`, {
    transform: (data) => (data && data.all_chats) ? data.all_chats : null,
  });
  const allChats = chatsResult.data ?? [];

  // Select the first chat once the list arrives (mirrors the old
  // fetch-then-select behavior — runs whenever the list is (re)loaded).
  useEffect(() => {
    if (chatsResult.data && chatsResult.data.length > 0) {
      setSelectedChatId(chatsResult.data[0].id);
    }
  }, [chatsResult.data]);

  const conversationResult = useApiData(
    selectedChatId ? `/api/v1/monitor/conversation/${selectedChatId}` : null,
    { transform: (data) => (data && data.conversation) ? data.conversation : null }
  );
  const monitorData = conversationResult.data ?? null;
  ...
  // Chats from the last 7 days, computed at render time (was previously
  // pinned to a hardcoded "today is 2026-06-25" literal).
  const weekAgoKey = daysAgo(7);
  const chatsThisWeek = allChats.filter(c => c.date >= weekAgoKey).length;
  ...
  const groupedChats = useMemo(() => {
    ...
    filteredChats.forEach(chat => {
      const dateLabel = chat.date === todayKey ? (isAr ? "اليوم" : "Today")
                      : chat.date === yesterdayKey ? (isAr ? "أمس" : "Yesterday")
                      : chat.date;

...
const KnowledgePage = ({ t, theme, language }) => {
  ...
  const fetchFiles = useCallback(async () => {
    try {
      const result = await apiFetch("/api/v1/data/list/0");
      if (result.ok) {
        ...
        console.error("Failed to fetch files. HTTP status:", result.status);
      }
    } catch (error) {
  ...
  const uploadFiles = useCallback(
    async (files) => {
      ...
      for (const file of files) {
        ...
        try {
          // FormData upload — doesn't go through apiFetch's JSON-body
          // signature, but still builds its URL from BASE_URL.
          const response = await fetch(
            `${BASE_URL}/api/v1/data/upload/${projectId}`,
  ...
  const deleteDoc = async (id) => {
    try {
      const projectId = "0";
      const path = `/api/v1/data/delete/${projectId}/${id}`;
      console.log("Calling DELETE endpoint:", `${BASE_URL}${path}`);
  ...
  // status → style map for this page's vocabulary (indexed/processing/error-fallback)
  const knowledgeStatusConfig = {
    indexed: {
      color: theme === "dark" ? "#4ade80" : "#166534",
      bg: theme === "dark" ? "rgba(34, 197, 94, 0.1)" : "#f0fdf4",
    },
    success: {
      color: theme === "dark" ? "#4ade80" : "#166534",
      bg: theme === "dark" ? "rgba(34, 197, 94, 0.1)" : "#f0fdf4",
    ...
    processing: {
      color: "#1A9BB3",
      bg: theme === "dark" ? "rgba(26, 155, 179, 0.1)" : "rgba(26, 155, 179, 0.05)",
    },
    indexing: {
      color: "#1A9BB3",
      bg: theme === "dark" ? "rgba(26, 155, 179, 0.1)" : "rgba(26, 155, 179, 0.05)",
    ...
    default: {
      ...
      bg: theme === "dark" ? "rgba(239, 68, 68, 0.1)" : "#fef2f2",
    },
  };
                ...
                >
                  ...
                  <td style={{ padding: "20px" }}>
                    <StatusBadge
                      variant="pill"
                      status={doc.status}
                      config={knowledgeStatusConfig}
                      label={t(doc.status) || doc.status}
                    />
                  </td>

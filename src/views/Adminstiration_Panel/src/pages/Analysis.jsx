...
const AnalysisPage = ({ t, theme, language }) => {
  ...
  const fetchTopics = async () => {
    try {
      const result = await apiFetch(`/api/v1/gap_analysis/topics/${projectId}`);
      if (result.ok) {
  ...
  const fetchWellCovered = async () => {
    try {
      const result = await apiFetch(`/api/v1/gap_analysis/well_covered/${projectId}`);
      if (result.ok) {
        const data = result.data;
  ...
  const fetchUnanswered = async () => {
    try {
      const result = await apiFetch(`/api/v1/gap_analysis/unanswered/${projectId}`);
      if (result.ok) {
        const data = result.data;
  ...
  const dismissQuery = async (queryId) => {
    ...
    try {
      const result = await apiFetch(
        `/api/v1/gap_analysis/dismiss/${projectId}/${queryId}`,
        { method: "PUT" }
      );
      if (result.ok) {
  ...
  const handleSuggestSubmit = async () => {
    ...
    try {
      const result = await apiFetch(
        `/api/v1/gap_analysis/suggest_document/${projectId}/${queryId}`,
        {
          method: "POST",
          headers: {

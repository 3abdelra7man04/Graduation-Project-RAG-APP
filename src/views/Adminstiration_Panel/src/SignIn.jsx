...
const SignInPage = ({ onSignIn, theme, toggleTheme }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await apiFetch("/api/v1/admin/login/1", {
        method: "POST",
      ...
      if (result.ok) {
        // Fetch profile
        const profileResult = await apiFetch(`/api/v1/admin/get-profile/1?admin_id=${result.data.admin_id}`);
        if (profileResult.ok) {
          onSignIn({
            userId: result.data.admin_id,
            email: email.trim(),
            profile: profileResult.data.adminData,
            projectId: "0"
          });
        } else {
